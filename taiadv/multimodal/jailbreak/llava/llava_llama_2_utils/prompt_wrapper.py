    ...
    def do_tokenization(self, text_prompts):
        ...
        if isinstance(text_prompts, list):
            for i in range(len(text_prompts)):
                input_id = tokenizer_image_token(text_prompts[i], self.tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).cuda()
                self.input_ids.append(input_id)
                self.context_length.append(input_id.shape[1])
            return
