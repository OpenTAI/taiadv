...
class PMA():
    def __init__(self, model, epsilon=8./255., num_steps=50,num_random_starts=1,
                 v_min=0., v_max=1., seed=0, norm='Linf', num_classes=100):
        self.model = model
            ...
            for r in range(2):
                ...
                for i in range(self.num_steps):
                    ...
                    if self.use_odi and i < 2:
                        ...
                        alpha = self.initial_step_size * 0.5 * (1 + .cos((i-change_point - 1) / (self.num_steps-change_point) * .pi))
                    else:
                        alpha = self.initial_step_size * 0.5 * (1 + .cos((i - 1) / (self.num_steps-change_point) * .pi))
