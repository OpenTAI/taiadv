...
class Attacker():
    def __init__(self, model, epsilon=8./255., v_min=0., v_max=1.,
        ...
        self.md_z_max = MD.MDAttackAblation(model, epsilon, num_steps=50, step_size=2/255,
                                            v_min=v_min, v_max=v_max, change_point=20,
                                            first_step_size=16./255., seed=0, norm='Linf',
                                            num_classes=num_classes, use_odi=False, term='z_max')
        self.md_z_y = MD.MDAttackAblation(model, epsilon, num_steps=50, step_size=2/255,
                                           v_min=v_min, v_max=v_max, change_point=20,
                                           first_step_size=16./255., seed=0, norm='Linf',
                                           num_classes=num_classes, use_odi=False, term='-z_y')
        self.md_z_max_z_y = MD.MDAttackAblation(model, epsilon, num_steps=50, step_size=2/255,
                                                v_min=v_min, v_max=v_max, change_point=20,
                                                first_step_size=16./255., seed=0, norm='Linf',
                                                num_classes=num_classes, use_odi=False, term='z_max-z_y')
        self.md_phase2 = MD.MDAttack(model, epsilon, num_steps=50, step_size=2/255,
                                     v_min=v_min, v_max=v_max, change_point=0,
                                     first_step_size=16./255., seed=0, norm='Linf',
                                     num_classes=num_classes, use_odi=False)
        ...
        self.fgsm = PGD.PGDAttack(model, epsilon, num_steps=1, step_size=8/255.,
        ...
        self.pgd = PGD.PGDAttack(model, epsilon, num_steps=20, step_size=0.8/255.,
                                 ...
                                 num_classes=num_classes, random_start=True,
                                 type='PGD', use_odi=False)
        self.pgd100 = PGD.PGDAttack(model, epsilon, num_steps=100, step_size=2/255.,
                                    num_restarts=1, v_min=v_min, v_max=v_max,
                                    num_classes=num_classes, random_start=True,
                                    type='PGD', use_odi=False)
        ...
        if version == 'MD':
            ...
            self.attacks_to_run = [self.md]
        elif version == 'MD_z_max':
            self.attacks_to_run = [self.md_z_max]
        elif version == 'MD_z_y':
            self.attacks_to_run = [self.md_z_y]
        elif version == 'MD_z_max_z_y':
            self.attacks_to_run = [self.md_z_max_z_y]
        elif version == 'MD_DLR':
        ...
        elif version == 'PGD4x100':
            self.attacks_to_run = [self.pgd100, self.pgd100, self.pgd100, self.pgd100]
        elif version == 'FGSM':
            self.attacks_to_run = [self.fgsm]
        elif version == 'MT':
