...
class Attacker():
    ...
    def evaluate(self):
        ...
        adv_i = set([i for i in range(total)])
        ...
        for i, a in enumerate(self.attacks_to_run):
            ...
            # Log
            if self.verbose:
                print(str(a)+f"adv_acc:{len(indexs)/total*100:.2f}")
                print(str(a)+f"time:{end-start}")
        indexs = indexs.cpu().tolist()
        indexs = set([i for i in indexs])
        adv_i = list(adv_i-indexs)
        adv_i = [str(i) for i in adv_i]
        return (clean_sum/total).item(), len(indexs)/total
    '''
