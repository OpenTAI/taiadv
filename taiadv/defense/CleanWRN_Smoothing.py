...
class CleanWRN_Smoothing(torch.nn.Module):
    destination = 'checkpoints/CleanWRN_Smoothing/'

    def __init__(self):
        super(CleanWRN_Smoothing, self).__init__()
        # Download pretrained weights
        filename = os.path.join(self.destination, 'WRN-34-10.pt')
        ...
        self.base_model = RobustWideResNet(
            num_classes=10, channel_configs=[16, 160, 320, 640],
            depth_configs=[5, 5, 5], stride_config=[1, 2, 2])
        self.base_model = self.base_model.to(device)
        msg = self.base_model.load_state_dict(checkpoint, strict=False)
        print(msg)
