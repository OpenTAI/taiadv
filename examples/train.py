...
def train(args, exp, model, optimizer, scheduler, criterion, loader,
    ...
    evaluator = Evaluator(
        criterion=criterion, data_loader=eval_loader, logger=logger)
    for epoch in range(start_epoch, config.epochs):
        ...
        model.train()
        exp_stats = trainer.train(
            epoch=epoch, model=model, optimizer=optimizer, exp_stats=exp_stats)
        scheduler.step()

        # adversarial evaluation
        logger.info('=' * 20 + 'Eval Epoch %d' % (epoch) + '=' * 20)
        model.eval()
        exp_stats = evaluator.adv_whitebox(
            model,
            epsilon=args.eps,
